...
# Для Python 3.11 и выше вместо timezone.utc используется UTC
...
class Opinion(db.Model):
    ...
    # На вход принимает словарь data, полученный из JSON в запросе.
    def from_dict(self, data):
        # Для каждого поля модели, которое можно заполнить...
        for field in ['title', 'text', 'source', 'added_by']:
            # ...выполняется проверка 
            # - есть ли ключ с таким же именем в словаре:
            if field in data:
